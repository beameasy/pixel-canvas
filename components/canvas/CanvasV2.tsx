...
const Canvas = forwardRef<CanvasRef, CanvasProps>(
  (
    ...
    // Track the canvas size so it can update when the container resizes
    const [canvasSize, setCanvasSize] = useState(600);

    // Update canvas size using a ResizeObserver
    useEffect(() => {
      const updateSize = () => {
    ...
    const [nextPlacementTime, setNextPlacementTime] = useState<number | null>(
      null,
    );
    ...
    const pixelUpdateQueue = useRef<
      { x: number; y: number; color: string; wallet_address: string }[]
    >([]);
    ...
    const processPixelUpdates = useCallback(() => {
      logger.log(
        "🔴 Processing pixel updates. Queue length:",
        pixelUpdateQueue.current.length,
      );
    ...
    const queuePixelUpdate = useCallback(
      (pixel: {
        x: number;
        y: number;
        color: string;
    ...
    useEffect(() => {
      ...
      channel.bind("pixel-placed", (data: PixelPlacedEvent) => {
        ...
        if (!pusherConnected || !data.pixel.wallet_address) {
          logger.log(
            "🔴 Pusher event ignored - not connected or missing wallet address",
          );
          return;
        }
      ...
      try {
        ...
        if (response.status !== 304) {
          ...
          setCanvasState((prev) => ({
            ...prev,
            pixels: new Map(
              data.map((pixel: any) => [`${pixel.x},${pixel.y}`, pixel]),
            ),
            isLoading: false,
          }));
        } else {
    ...
    const drawSinglePixel = useCallback(
      (x: number, y: number, color: string) => {
        ...
        if (canvasState.view.scale > 4) {
          ...
          ctx.lineTo(
            screenX,
            (y + 1) * PIXEL_SIZE * canvasState.view.scale + canvasState.view.y,
          );
          ctx.stroke();
          ...
          ctx.lineTo(
            (x + 1) * PIXEL_SIZE * canvasState.view.scale + canvasState.view.x,
            screenY,
          );
          ctx.stroke();
        }
      ...
      const loadInitial = async () => {
        try {
          ...
          const shouldForceRefresh = pixelPlacedAt
            ? Date.now() - parseInt(pixelPlacedAt) < 60000
            : false;
      ...
      () => ({
        ...
        setViewState: (viewState: ViewState) => {
          ...
          const safeScale = Math.min(
            MAX_ZOOM,
            Math.max(MIN_ZOOM, viewState.scale),
          );
    ...
    const handleMouseDown = (e: React.MouseEvent) => {
      ...
      const x = Math.floor(
        (mouseX - canvasState.view.x) / (PIXEL_SIZE * canvasState.view.scale),
      );
      const y = Math.floor(
        (mouseY - canvasState.view.y) / (PIXEL_SIZE * canvasState.view.scale),
      );
    ...
    const handleMouseMove = useCallback(
      (e: React.MouseEvent) => {
        ...
        const x = Math.floor(
          (canvasX - canvasState.view.x) /
            (PIXEL_SIZE * canvasState.view.scale),
        );
        const y = Math.floor(
          (canvasY - canvasState.view.y) /
            (PIXEL_SIZE * canvasState.view.scale),
        );
      ...
      return () => {
        ...
        if (canvasState.view.scale > 4) {
          ...
          for (let x = 0; x <= GRID_SIZE; x++) {
            ...
            ctx.lineTo(
              screenX,
              GRID_SIZE * PIXEL_SIZE * canvasState.view.scale +
                canvasState.view.y,
            );
            ctx.stroke();
          }
      ...
      if (!canvasState.isLoading) {
        try {
          localStorage.setItem(
            "canvasViewState",
            JSON.stringify(canvasState.view),
          );
        } catch (error) {
          logger.error("Failed to save canvas view state:", error);
        }
    ...
    const fetchBalance = useCallback(
      async (force = false) => {
        ...
        if (
          !force &&
          now - lastBalanceFetchTime.current < BALANCE_FETCH_COOLDOWN
        ) {
        ...
        try {
          logger.log(
            `🔵 Fetching balance for: ${address}${force ? " (forced)" : ""}`,
          );
          const token = await getAccessToken();
    ...
    const handlePlacePixel = async (x: number, y: number, color: string) => {
      ...
      if (x < 0 || y < 0 || x >= GRID_SIZE || y >= GRID_SIZE) {
        ...
        logger.log(
          `Ignoring placement attempt at invalid coordinates (${x}, ${y})`,
        );
        return;
      }
        ...
        if (e.touches.length === 2) {
          ...
          if (pinchStart > 0) {
            ...
            const scale = Math.min(
              MAX_ZOOM,
              Math.max(MIN_ZOOM, pinchScale * (distance / pinchStart)),
            );
      ...
      (e: WheelEvent) => {
        ...
        setCanvasState((prev) => {
          ...
          const newScale = Math.min(
            MAX_ZOOM,
            Math.max(MIN_ZOOM, prev.view.scale * scaleChange),
          );
    ...
    const handleReactTouchStart = (e: React.TouchEvent) => {
      ...
      const touch = e.touches[0];
      const x = Math.floor(
        (touch.clientX - rect.left - canvasState.view.x) /
          (PIXEL_SIZE * canvasState.view.scale),
      );
      const y = Math.floor(
        (touch.clientY - rect.top - canvasState.view.y) /
          (PIXEL_SIZE * canvasState.view.scale),
      );
    ...
    const handleSelectionStart = useCallback(
      (e: MouseEvent) => {
        ...
        const x = Math.floor(
          (e.clientX - rect.left - canvasState.view.x) /
            (PIXEL_SIZE * canvasState.view.scale),
        );
        const y = Math.floor(
          (e.clientY - rect.top - canvasState.view.y) /
            (PIXEL_SIZE * canvasState.view.scale),
        );
    ...
    const handlePixelPlacement = async (
      x: number,
      y: number,
      color: string,
    ) => {
      try {
        ...
        if (
          ...
          logger.error("⚠️ Invalid coordinates:", {
            originalX: x,
            originalY: y,
            gridX,
            gridY,
          ...
          setFlashMessage(
            `Invalid coordinates (${gridX}, ${gridY}). Must be between 0 and ${GRID_SIZE - 1}.`,
          );
          clearPreviewPixel();
          return false;
        }
        ...
        logger.log("🎨 Starting pixel placement:", {
          x: gridX,
          y: gridY,
          color,
        });
        ...
        if (!response.ok) {
          ...
          if (response.status === 409 && error.currentVersion) {
            ...
            setFlashMessage(
              "Pixel was modified by someone else. Canvas refreshed.",
            );
            ...
            setFlashMessage(
              error.error || "Your wallet has been banned from placing pixels",
            );
            setFlashHasLink(false);
        ...
        logger.log(
          `🕒 Cooldown set: ${cooldownMs / 1000}s until next pixel placement`,
        );
        ...
        setFlashMessage(
          error instanceof Error ? error.message : "Failed to place pixel",
        );
    ...
    useEffect(() => {
      ...
      const visibilityHandler = async () => {
        if (document.visibilityState === "visible") {
          ...
          if (timeAway > 30000) {
            logger.log(
              "Tab was away for more than 30 seconds, reconnecting Pusher",
            );
            ...
            pusherManager.subscribe("pixel-placed", (data: any) => {
              ...
              setCanvasState((prev) => ({
                ...prev,
                pixels: new Map(prev.pixels).set(
                  `${pixel.x},${pixel.y}`,
                  pixel,
                ),
              }));
            ...
            try {
              const response = await fetch(
                "/api/pixels/recent?since=" +
                  Math.floor(lastVisibilityTime / 1000),
              );
              if (response.ok) {
                const data = await response.json();
    ...
    const BILLBOARD_TOKEN_URL =
      "https://clank.fun/t/0x0ab96f7a85f8480c0220296c3332488ce38d9818";

    // Add a function to determine if the user can place a pixel at the current hover position
    const canPlacePixelAtPosition = useCallback(
      (
        x: number,
        y: number,
      ): {
        ...
        if (!authenticated) {
          return {
            canPlace: false,
            reason: "Sign in to place pixels",
            hasHtml: false,
            needsTokens: false,
          };
        }
        ...
        if (isBanned) {
          return {
            canPlace: false,
            reason: "Your wallet has been banned",
            hasHtml: false,
            needsTokens: false,
          };
        }
        ...
        if (nextPlacementTime && Date.now() < nextPlacementTime) {
          const secondsLeft = Math.ceil(
            (nextPlacementTime - Date.now()) / 1000,
          );
          return {
            canPlace: false,
            reason: `Cooldown: wait ${secondsLeft}s`,
            hasHtml: false,
            needsTokens: false,
          };
        }
        ...
        if (
          existingPixel.locked_until &&
          Number(existingPixel.locked_until) > Date.now()
        ) {
        ...
        if (
          ownerWallet &&
          userProfiles[ownerWallet]?.profile?.token_balance !== undefined
        ) {
        ...
        const pixelAge =
          Date.now() - new Date(existingPixel.placed_at).getTime();
        ...
        if (pixelAge < protectionTimeMs && ownerTier.protectionTime > 0) {
          ...
          if (currentUserBalance > ownerBalance) {
            ...
            const hoursLeft = Math.ceil(
              (protectionTimeMs - pixelAge) / (60 * 60 * 1000),
            );
    ...
    const [needTokensPosition, setNeedTokensPosition] = useState({
      x: 0,
      y: 0,
    });
    ...
    const handleRealTimePixelUpdate = useCallback(
      (data: any) => {
    ...
    useEffect(() => {
      ...
      const visibilityHandler = async () => {
        if (document.visibilityState === "visible") {
          ...
          if (timeAway > 10000) {
            logger.log(
              "Tab was away for more than 10 seconds, fetching recent pixels",
            );
    ...
    return (
      ...
      >
        ...
        {showBuyTokensButton && (
          <div className="fixed bottom-24 left-1/2 transform -translate-x-1/2 z-[100] animate-bounce">
            <button
        ...
        {flashMessage && (
          <div className="fixed left-1/2 top-1/3 transform -translate-x-1/2 -translate-y-1/2 z-[100]">
            <div
              onClick={() =>
                flashLinkAction.current && flashLinkAction.current()
              }
            ...
            >
              <div className="flex items-center gap-2 text-xs">
                {hoverData?.pixel?.farcaster_pfp &&
                  hoverData.pixel.farcaster_pfp !== "null" && (
                    <>
                ...
                <span
                  className={
                    ...
                    hoverData.pixel.farcaster_username !== "null"
                      ? "text-purple-400"
                      : "text-blue-400"
                  }
                >
              ...
              <div className="text-yellow-400 text-xs">
                {hoverData?.pixel?.wallet_address &&
                userProfiles[hoverData.pixel.wallet_address]
                  ? formatBillboardAmount(
                      userProfiles[hoverData.pixel.wallet_address].profile
                        ?.token_balance ?? 0,
                    )
              ...
              {hoverData?.pixel?.locked_until &&
                Number(hoverData.pixel.locked_until) > Date.now() && (
                  <div className="text-yellow-400 mt-0.5 text-xs">
                    🔒 Locked for{" "}
                    {formatTimeSince(
                      new Date(
                        Number(hoverData.pixel.locked_until),
                      ).toISOString(),
                    )}
                  </div>
                )}
              ...
              {authenticated && (
                <div className="mt-2 pt-1 border-t border-neutral-700/50">
                  ...
                  >
                    {canPlacePixelAtPosition(hoverData.x, hoverData.y)
                      .hasHtml ? (
                      <div
                        dangerouslySetInnerHTML={{
                          __html: canPlacePixelAtPosition(
                            hoverData.x,
                            hoverData.y,
                          ).reason,
                        }}
            ...
            {authenticated && (
              ...
              >
                {canPlacePixelAtPosition(hoverData.x, hoverData.y).hasHtml ? (
                  <div
                    dangerouslySetInnerHTML={{
                      __html: canPlacePixelAtPosition(hoverData.x, hoverData.y)
                        .reason,
                    }}
                  />
                ) : (
        ...
        {!canvasState.isLoading && !isSmallScreen && windowSize.width > 0 && (
          <div className="absolute bottom-4 right-4 z-50">
            <Minimap
              ...
              pixels={
                new Map(
                  [...canvasState.pixels].map(([key, pixel]) => [
                    key,
                    pixel.color,
                  ]),
                )
        ...
        <div className="absolute top-2 right-2 z-50 bg-neutral-900/90 rounded-lg px-2 py-1 text-xs font-mono flex flex-col items-end gap-0.5">
          <div className="flex items-center">
            <div className="text-amber-400 flex-grow whitespace-nowrap">
              {formatBillboardAmount(userProfile?.token_balance || 0)}
              {isSmallScreen ? " BB" : " $BILLBOARD"}
            </div>
            <button
          ...
          <div
            className={`${!nextPlacementTime ? "text-green-400" : "text-neutral-400"} text-[10px] leading-tight`}
          >
            {!nextPlacementTime
              ? isSmallScreen
                ? "Ready!"
